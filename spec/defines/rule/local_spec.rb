    ...
    on_supported_os.each do |os, os_facts|
      context "on #{os}" do
        ...
        context 'when only rule and target_log_file specified' do
          ...
          it do
            is_expected.to contain_rsyslog__rule('99_simp_local/test_name.conf').with_content(
              <<~EOM,
        ...
        context 'when full-path dyna_file specified' do
          ...
          it do
            is_expected.to contain_rsyslog__rule('99_simp_local/test_name.conf').with_content(
              %r{template\(name="#{title}" type="string" string="#{params[:dyna_file]}"\)\n\nif \(#{params[:rule]}\) then\s+\{\n\s*action\(\n\s*type="omfile"\n\s*dynaFile="#{title}"},
            )
          end
        end
